...
class DenseGGNNChemModel(ChemModel):
    ...
    def default_params(cls):
        ...
        params.update({
                        ...
                        'hidden_size': 100,        
                        "kl_trade_off_lambda": 5,    # kl tradeoff
                        'learning_rate': 0.001, 
                        'graph_state_dropout_keep_prob': 1,    
                        "compensate_num": 1,           # how many atoms to be added during generation

                        'train_file': 'data/selected_train_data_%s.json' % dataset,
                        'valid_file': 'data/selected_valid_data_%s.json' % dataset,
                        ...
                        "z_verbose": False,    # only for output z
                        })
    ...
    def compute_mean_and_logvariance(self):
        ...
        mean = tf.reshape(mean, [-1, v, h_dim])
        logvariance = tf.reshape(logvariance, [-1, v, h_dim])
        mean = tf.reduce_mean(mean, 1)
        logvariance = tf.reduce_mean(logvariance, 1)
    ...
    def sample_with_mean_and_logvariance(self):
        ...
        self.ops['z_graph'] = z_sampled
        # convert graph-level z to node-level
        z_sampled = tf.broadcast_to(tf.reshape(z_sampled,[batch_size,1,h_dim]), [batch_size,v,h_dim])
        # filter
    ...
    def construct_loss(self):
        ...
        kl_loss = tf.broadcast_to(tf.reshape(kl_loss,[batch_size,1,h_dim]), [batch_size, v, h_dim])
        kl_loss = tf.reshape(kl_loss, [-1, v, h_dim]) * self.ops['graph_state_mask'] 
        self.ops['kl_loss'] = -0.5 * tf.reduce_sum(kl_loss, [1,2])
        with tf.variable_scope("polynomial_regression"):
            for degree_id in range(3):
                self.weights['poly%i'%degree_id] = tf.Variable(1.0, name='poly_weight%i'%degree_id)
        z_0_prime = self.ops['mean'][:,0]
        z_0_prime = self.weights['poly0'] + self.weights['poly1']*z_0_prime + self.weights['poly2']*tf.pow(z_0_prime, 2)
        self.ops['z_0_prime'] = z_0_prime
        sup_loss = tf.keras.losses.mean_squared_error(tf.broadcast_to(tf.reshape(z_0_prime,[-1,1]),[batch_size,v]), self.placeholders['weight'])
        self.ops['sup_loss'] = tf.reduce_sum(sup_loss)
        z_0_prime = self.ops['mean'][:,0]
        poly_der = 2*z_0_prime*self.weights['poly2']+self.weights['poly1']
        mono_loss = tf.nn.relu(2*z_0_prime*self.weights['poly2']+self.weights['poly1'])
        self.ops['mono_loss'] = 2*tf.reduce_sum(mono_loss)
        # Node symbol loss
        ...
        return tf.reduce_mean(self.ops["edge_loss"] + self.ops['node_symbol_loss'] + \
                              ...
                              + self.ops['sup_loss'] + self.ops['mean_total_qed_loss'] + self.ops['mono_loss']
    ...
    def process_raw_graphs(self, raw_data, is_training_data, file_name, bucket_sizes=None):
        ...
        for d, (incremental_adj_mat,distance_to_others,node_sequence,edge_type_masks,edge_type_labels,local_stop, edge_masks, edge_labels, overlapped_edge_features)\
            ...
            bucketed[chosen_bucket_idx].append({
                ...
                'PSA': [d['PSA']],
                'rPSA': [d['rPSA']],
                'logP': check_logp_by_smiles([d['smiles']]),
                'weight': [d['weight']],
    ...
    def make_batch(self, elements, maximum_vertice_num):
        ...
        batch_data = {'adj_mat': [], 'init': [], 'labels': [], 'edge_type_masks':[], 'edge_type_labels':[], 'edge_masks':[],                 
                ...
                'max_iteration_num': max_iteration_num, 'overlapped_edge_features': [], 'weight':[], 'logP':[], 'smiles':[], 'logS':[], 'Drug':[], 'PSA':[], 'rPSA':[]}
        for d in elements: 
            ...
            batch_data['weight'].append(d['weight'])
            batch_data['logP'].append(d['logP'])
            batch_data['logS'].append(d['logS'])
            batch_data['Drug'].append(d['Drug'])
            batch_data['PSA'].append(d['PSA'])
            batch_data['rPSA'].append(d['rPSA'])
    ...
    def generate_graph_with_state(self, random_normal_states, num_vertices,
        ...
        for idx in starting_point: 
            # generate a new molecule
            new_mol, total_log_prob=self.search_and_generate_molecule(idx, np.copy(valences), sampled_node_symbol, real_length,random_normal_states, elements, num_vertices)
    ...
    def generate_new_graphs(self, data): 
        ...
        for step in range(len(bucket_at_step)):
            ...
            for elements in elements_batch:
                ...
                # initial state
                random_normal_states=generate_std_normal(1, 1,\
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                self.placeholders['smiles']: batch_data['smiles'],
                self.placeholders['logS']: batch_data['logS'],
                self.placeholders['Drug']: batch_data['Drug'],
                self.placeholders['PSA']: batch_data['PSA'],
                self.placeholders['rPSA']: batch_data['rPSA'],
                self.placeholders['logP']: batch_data['logP'],
                self.placeholders['weight']: batch_data['weight'],
