...
class ChemModel(object):
    ...
    def make_model(self):
        ...
        # logp condition for this batch
        self.placeholders['logP'] = tf.placeholder(tf.float32, [None, 1], name='logP') # [b, 1]
        # weight 
        self.placeholders['weight'] = tf.placeholder(tf.float32, [None, 1], name='weight') # [b, 1]
        # smiles
        self.placeholders['smiles'] = tf.placeholder(tf.string, [None, 1], name='smiles') # [b, 1]
        # logs
        self.placeholders['logS'] = tf.placeholder(tf.float32, [None, 1], name='logS') # [b, 1]
        # drug
        self.placeholders['Drug'] = tf.placeholder(tf.float32, [None, 1], name='Drug') # [b, 1]
        # PSA
        self.placeholders['PSA'] = tf.placeholder(tf.float32, [None, 1], name='PSA') # [b, 1]
        # rPSA
        self.placeholders['rPSA'] = tf.placeholder(tf.float32, [None, 1], name='rPSA') # [b, 1]
    ...
    def run_epoch(self, epoch_name: str, epoch_num, data, is_training: bool):
        ...
        for step, batch_data in enumerate(batch_iterator):
            ...
            batch_data[self.placeholders['z_prior']] = utils.generate_std_normal(\
                self.params['batch_size'],1, self.params['hidden_size'])
            if is_training:
                ...
                fetch_list = [self.ops['loss'], self.ops['train_step'], 
                              self.ops["edge_loss"],self.ops['node_symbol_loss'], self.ops['kl_loss'], 
                              ...
                              #self.ops['mean_kl_loss'], self.ops['mean_total_qed_loss'],self.placehold['num_vertices'],self.weights['poly0'],self.weights['poly1'],self.weights['poly2'],self.weights['poly3'],self.ops['sup_loss'],self.ops['mono_loss']]
                              self.ops['mean_kl_loss'], self.ops['mean_total_qed_loss'],self.placeholders['num_vertices'],self.weights['poly0'],self.weights['poly1'],self.weights['poly2'],self.ops['sup_loss']]
            else:
        ...
        with self.graph.as_default():
            for epoch in range(1, self.params['num_epochs'] + 1):
                if not self.params['generation']:
                    ...
                    self.save_model(str(epoch)+("_%s_poly_mono_Dln.pickle" % (self.params["dataset"])))
